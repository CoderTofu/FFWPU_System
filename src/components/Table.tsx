...
const Table: React.FC<TableProps> = ({ data, columns, rowDoubleClickPath, onRowSelect}) => {
  ...
  const handleRowDoubleClick = (row: any) => {
    if (rowDoubleClickPath) {
      router.push(`${rowDoubleClickPath}/${row.ID}`); // Redirect with ID in URL
    }
  };
  ...
  return (
    <div ref={tableRef} className="overflow-x-auto min-h-[300px] max-h-[450px] overflow-y-auto rounded-lg">
      <table className="min-w-full border-gray-300 bg-white rounded-lg">
        ...
        <tbody>
          {data.map((row, index) => (
            <tr key={index} className={`cursor-pointer transition
