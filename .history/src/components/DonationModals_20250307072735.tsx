...
export default function DonationModals({
  ...
  return (
    <div className="flex items-center gap-4 w-full">
      <div className="relative w-full sm:w-[250px] md:w-[300px] lg:w-[500px] min-w-[150px]">
      ...
      <div ref={dropdownRef} className="flex flex-row items-center gap-4">
        {["Member ID", "Date", "Church"].map((filter) => (
          <div key={filter} className="relative">
            <button
              onClick={() => toggleSortDropdown(openSortDropdown === filter ? null : filter)}
              className="w-[120px] min-w-[100px] md:w-[140px] lg:w-[160px] h-10 border border-[#01438F] rounded-md px-3 outline-none bg-white text-gray-600 flex justify-between items-center"
            >
              {filter}
