...
export default function Donation() {
  ...
  // Helper function to get the numeric value in PHP
  const getAmountInPHP = (amount: string) => {
    const [currency, value] = amount.split(" ");
    const numericValue = parseFloat(value);
    if (currency === "USD") {
      return numericValue * 55; // Example: 1 USD = 55 PHP
    }
    return numericValue;
  };

  const handleSort = (order: "asc" | "desc") => {
    // Separate the data into USD and PHP donations
    const usdDonations = originalData.filter(item => item.Amount.startsWith("USD"));
    const phpDonations = originalData.filter(item => item.Amount.startsWith("PHP"));

    // Sort USD donations by amount in descending order after converting to PHP
    const sortedUSDDonations = usdDonations.sort((a, b) => {
      const aAmountInPHP = getAmountInPHP(a.Amount);
      const bAmountInPHP = getAmountInPHP(b.Amount);
      return order === "asc" ? aAmountInPHP - bAmountInPHP : bAmountInPHP - aAmountInPHP;
    });

    // Sort PHP donations by amount in descending order
    const sortedPHPDonations = phpDonations.sort((a, b) => {
      const aAmount = parseFloat(a.Amount.split(" ")[1]);
      const bAmount = parseFloat(b.Amount.split(" ")[1]);
      return order === "asc" ? aAmount - bAmount : bAmount - aAmount;
    });

    // Combine USD and PHP donations, USD first
    const finalSortedData = [...sortedUSDDonations, ...sortedPHPDonations];
    ...
    <div className="bg-[#D9D9D9] min-h-screen flex flex-col">
      ...
      <div className="overflow-hidden rounded-lg mt-5 mx-auto sm:w-[90%] md:w-[90%] lg:w-[90%] xl:w-[96%] items-center justify-center">
        <div className="bg-white">
          <Table data={sortedData} columns={{ lg: ["Donation ID", "Member ID", "Name", "Date", "Church", "Amount"] }} onRowSelect={setSelectedRow} />
        </div>
