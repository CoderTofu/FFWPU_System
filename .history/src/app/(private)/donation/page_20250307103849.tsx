...
export default function Donation() {
  ...
  const handleSort = (key: keyof DataItem, order: "asc" | "desc") => {
    const sorted = [...originalData].sort((a, b) => {
      if (key === "Amount") {
        // Extract numeric value and currency for sorting
        const parseAmount = (amount: string) => {
          const regex = /([a-zA-Z]+)\s(\d+(\.\d+)?)/; // Match currency + number (e.g., 'PHP 100.50')
          const match = amount.match(regex);
          if (match) {
            return {
              currency: match[1], // 'PHP' or 'USD'
              numericValue: parseFloat(match[2]), // 100.50
            };
          }
          return { currency: "", numericValue: 0 }; // Fallback case
        };
        ...
        // Handle other fields (non-Amount) for sorting
