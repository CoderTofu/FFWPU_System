...
export default function Donation() {
  ...
  const [currency, setCurrency] = useState<string>("USD"); // State to handle selected currency
  ...
  // Handle sorting logic for different columns
  const handleSort = (key: keyof DataItem, order: "asc" | "desc") => {
    const sorted = [...sortedData].sort((a, b) => {
      ...
      // Handle sorting for number values
      if (typeof aValue === "number" && typeof bValue === "number") {
        return order === "asc" ? aValue - bValue : bValue - aValue;
      }
  
      // If neither is a string nor number, return 0 (no sorting)
      return 0;
    });
