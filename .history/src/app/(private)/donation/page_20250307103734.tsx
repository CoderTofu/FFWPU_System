...
export default function Donation() {
  ...
  const handleSort = (key: keyof DataItem, order: "asc" | "desc") => {
    const sorted = [...originalData].sort((a, b) => {
      if (key === "Amount") {
        // Extracting amount value and currency
        const parseAmount = (amount: string) => {
          const currency = amount.split(" ")[0];  // Extract currency
          const numericAmount = parseFloat(amount.replace(/[^0-9.]/g, '')); // Extract numeric value
          return { numericAmount, currency };
        };
  
        const aParsed = parseAmount(a.Amount);
        const bParsed = parseAmount(b.Amount);
  
        // Sorting based on numeric value first
        if (order === "asc") {
          return aParsed.numericAmount - bParsed.numericAmount;
        } else {
          return bParsed.numericAmount - aParsed.numericAmount;
        }
      } else {
