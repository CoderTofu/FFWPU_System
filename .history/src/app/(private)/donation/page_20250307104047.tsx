    ...
    const sorted = [...originalData].sort((a, b) => {
      if (key === "Amount") {
        const parseAmount = (amount: string) => {
          // Regex to separate the currency and numeric value
          const regex = /([a-zA-Z]+)\s(\d+(\.\d+)?)/;
          const match = amount.match(regex);
          if (match) {
            return {
              currency: match[1], // 'PHP', 'USD', etc.
              numericValue: parseFloat(match[2]), // 100.50, 250.00, etc.
            };
          }
          return { currency: "", numericValue: 0 };
        };
  
        const aAmount = parseAmount(a.Amount);
        const bAmount = parseAmount(b.Amount);
  
        // Sort by numeric value (ignore the currency for comparison)
        if (order === "asc") {
          return aAmount.numericValue - bAmount.numericValue;
        } else {
          return bAmount.numericValue - aAmount.numericValue;
        }
      } else {
        // Default sorting for non-Amount fields (like Date, Name, etc.)
