...
export default function Donation() {
  ...
  // Function to format the amount as currency using Intl.NumberFormat
  const formatCurrency = (amount: string) => {
    ...
    return amount; // Fallback if no match found
  };
    ...
    <div className="bg-[#D9D9D9] min-h-screen flex flex-col">
      ...
      <div className="w-full max-w-6xl mt-4">
        <table className="table-auto w-full border-collapse">
          ...
          <tbody>
            {sortedData.map((row) => (
              <tr key={row["Donation ID"]} className="border-b">
                <td className="px-4 py-2">{row["Donation ID"]}</td>
                <td className="px-4 py-2">{row["Member ID"]}</td>
                <td className="px-4 py-2">{row.Name}</td>
                <td className="px-4 py-2">{row.Date}</td>
                <td className="px-4 py-2">{row.Church}</td>
